...
function navstatus(q, n, o) {
	...
	o = +o + +n; // use unary plus operator to add strings by converting to numbers
	itemTemplates.push(html`<a href="/csdco.html?q=${q}&n=${n}&o=0"> back </a>`);
	itemTemplates.push(html`<a href="/csdco.html?q=${q}&n=${n}&o=${o}"> >> </a>`);
...
function updateURL(q, n, o) {
	...
	document.title = `CSDCO Search:${q}&n=${n}&o=${o}`;
...
const showresults = (content) => {
	...
	for (var i = 0; i < count; i++) {
		...
		if (getSafe(() => barval[i].type.value)) {
			if (barval[i].type.value == "http://www.schema.org/DigitalDocument") {
