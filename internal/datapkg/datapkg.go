...
func parseManifest(j []byte) DataPackage {
	...
	// TODO..   need a hack here while we await update to the package generation code
	// to scope the Class and associated properties for a file into the schema.org JSON-LD
	//  Read the file name, match to the report catalog and get the type..
	//  Associate the type with a class, and pull and include the
	// information in this struct...  (will be passed to the template for display
	//  -- Real version will be a web component that reads this from the JSON-LD and displays at DOM render
